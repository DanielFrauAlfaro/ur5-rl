...
if __name__ == "__main__":
    ...
    while True:
        ...
        if True:
            __ = (dq_distance(torch.tensor(DQ_obj).unsqueeze(dim=0), torch.tensor(DQ_w).unsqueeze(dim=0)))
        else:
            # print("Euclidean: ", np.linalg.norm(np.array(pos_w) - np.array(pos_obj_)))
            __ = (dq_distance(torch.tensor(DQ_obj_).unsqueeze(dim=0), torch.tensor(DQ_w).unsqueeze(dim=0)))
