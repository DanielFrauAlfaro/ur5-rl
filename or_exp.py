...
for i in range(1000):
    new_p = copy.deepcopy(or_original)
    new_p[idx] += i / 200
    new_p[1] += i / 200
    new_p[2] += i / 200

    
    euc_d = np.linalg.norm(np.array(new_p) - np.array(or_original))
    # print(p_original)
    # print("--")
    ...
    # new_p.append(0.0)
    pos_wQ = get_quaternion(p_aux)
    orn_w = p.getQuaternionFromEuler(new_p)
    orn_wQ = get_quaternion(orn_w)
    DQ_w = get_dualQuaternion(q_r=orn_wQ, q_t=pos_wQ)
    w_DQ_vec = dqrobotics.vec8(DQ_w)

    __, __, DQ_d = dq_distance(torch.tensor(np.array([w_DQ_vec])), torch.tensor(np.array([original_DQ_vec])))
...
'''
En este caso la pendiente de la distancia calculada con los angulos de euler es menor.
En cambio, se observa no linealidad en el cálculo de la distancia con los cuaterniones en el caso de 
